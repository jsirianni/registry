...
[![CI](https://github.com/jsirianni/registry/actions/workflows/ci.yml/badge.svg)](https://github.com/jsirianni/registry/actions/workflows/ci.yml)
...
**Add Terraform Provider Version**

Returns a `202` when a new version is created and a `200` if an existing version has been updated.

```
PUT /v1/:namespace/:name/version
```
```bash
curl -v -XPUT -d @provider_version.json localhost:8080/v1/hashicorp/random/versions | jq
```

**Downloading**

You can download providers with Terraform

```tf
terraform {
  required_providers {
    random = {
      source  = "localhost/hashicorp/random"
      version = "~> 2.0.0"
    }
  }
}
...
In this example, it is assumed that the server is running on localhost with TLS.

## Provider Version

Provider versions should adhear to the following json structure:

<details>
<summary>provider_version.json</summary>
    ...
    "platforms": [
        {
            ...
            "shasums_signature_url": "https://releases.hashicorp.com/terraform-provider-random/2.0.0/terraform-provider-random_2.0.0_SHA256SUMS.72D7468F.sig",
